...
object DatabaseInitializer {
    ...
    suspend fun ensureEncyclopediaEntries(db: AppDatabase) {
        withContext(Dispatchers.IO) {
            // Удаляем старые данные из таблицы encyclopedia
            db.plantCareDao().deleteAllEncyclopediaEntries()
            println("DEBUG: Old encyclopedia data deleted.") // Отладка

            // Вставляем новые записи из списка
            db.plantCareDao().insertAllEncyclopediaEntries(encyclopediaEntries)
            println("DEBUG: New encyclopedia data inserted.") // Отладка
        }
    }

    // --- Список энциклопедии ---
    private val encyclopediaEntries = listOf(
        EncyclopediaEntry(
            id = 1,
